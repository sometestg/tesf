...
if game.CoreGui:FindFirstChild("DeltaGui") then
    game.CoreGui:FindFirstChild("DeltaGui"):Destroy()
end
...
DELTA["c0"]["Text"] = "-- Welcome to Delta!\nUse '/e sd' to show Delta and '/e hd' to hide Delta.";
...
function whitelist()
    ...
    local response = game:HttpGet("https://whitelist.deltaexploits.com/api/whitelisted?username=" .. username)
...
function checkkey()
    ...
    if(savedkey == "WHITELIST") then
        whitelist()
    elseif string.find(game:HttpGet("https://redirect-api.work.ink/tokenValid/" .. savedkey), "true") then
        GrantAccess()
    else
...
local url = "https://www.scriptblox.com/api/script/search?q=?"
...
--print"Searching"
    ...
    local url = "https://www.scriptblox.com/api/script/search?q="..KeyWordSearch
    local response = game:HttpGetAsync(url)
    local http = game:GetService("HttpService")
    local decoded = http:JSONDecode(response)
    for _, script in pairs(decoded.result.scripts) do
...
btns.Button3.MouseButton1Click:Connect(function()
    UILib.SavedScripts:Add(lib:GetSelectedScriptTitle(), lib:GetSelectedScript())
...
ScriptSuggestion.Overlay.Holder.Showcase.MouseButton1Click:Connect(function()
    getgenv().ExecuteSuggestedScript()
    end)
end;
task.spawn(C_122);
-- StarterGui.Delta.Executor.Sidemenu.Network.NetworkStatsHandler
...
UILib.Settings:AddDropdown("Change FPS Cap", "Change the FPS cap for a smoother experience", "60 FPS", {"60 FPS", "120 FPS", "Max FPS"}, function(selection)
